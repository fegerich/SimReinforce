...
TOTAL_TIMESTEPS = 500_000    # Demo-Wert; für gute Ergebnisse: >= 2_000_000
SEED            = 42
...
def evaluiere(policy, label: str, n: int = 2) -> None:
    ...
    print(f"  {label:<22s}  Ø Reward {r:8.1f}  |  "
          f"Abgeliefert {d:6.1f}  |  Treppenhaus {s:5.1f}")
...
print(f"""
─── RL-Modell in der echten Simulation verwenden ───
    from stable_baselines3 import PPO
    from Model.strategie import RLStrategie
    from simulation import main

    main(strategie=RLStrategie(PPO.load(r"{best_oder_final}")))
""")
